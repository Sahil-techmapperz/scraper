...
class NaukriParser
{
    ...
    {
        $decoded = $this->decode($payload);
        $item = $decoded['data'] ?? $decoded['listing'] ?? $decoded['item'] ?? $decoded['job'] ?? $decoded;

        if (! is_array($item) || array_is_list($item)) {
            throw new SourceUnavailableException('Listing detail payload did not contain a valid Naukri job object.', 'PARSER_FAILURE');
        }
    ...
    private function extractList(array $decoded): array
    {
        ...
        throw new SourceUnavailableException('Search payload did not contain a Naukri listing collection.', 'PARSER_FAILURE');
    }
}
