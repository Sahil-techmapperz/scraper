...
class Cache extends BaseConfig
{
    ...
    public function __construct()
    {
        ...
        $this->handler       = (string) env('CACHE_HANDLER', $this->handler);
        $this->backupHandler = (string) env('CACHE_BACKUP_HANDLER', $this->backupHandler);

        $this->redis['host']     = (string) env('REDIS_HOST', $this->redis['host'] ?? '127.0.0.1');
        $this->redis['port']     = (int) env('REDIS_PORT', $this->redis['port'] ?? 6379);
        $password                = env('REDIS_PASSWORD');
        $this->redis['password'] = ($password !== null && $password !== '') ? (string) $password : null;
        $this->redis['database'] = (int) env('REDIS_DATABASE', $this->redis['database'] ?? 0);
    }
}
